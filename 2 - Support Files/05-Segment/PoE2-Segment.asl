...
startup
{
    ...
    vars.lastAreaId = "";
    vars.areaRegex = new System.Text.RegularExpressions.Regex("^[^ ]+ [^ ]+ (\\d+)(?=.*(?:\\s2caa[0-9A-Fa-f]{4}\\s|Generating level))(?=.*\\[DEBUG\\s+[^\\]]+\\]\\s+.*?(\\d{1,3})\\D+\"([A-Za-z][A-Za-z0-9_]*)\")", System.Text.RegularExpressions.RegexOptions.Compiled | System.Text.RegularExpressions.RegexOptions.IgnoreCase);
