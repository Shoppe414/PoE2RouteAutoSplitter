...
The executable is built locally; this source package does not contain a precompiled Windows binary.
...
`Build.ps1` uses literal PowerShell filesystem paths so the bundled directory name `BossWatcher [Boss Rush Detection]` is safe. Square brackets are otherwise wildcard syntax for cmdlets such as `Test-Path` and `Copy-Item`.
