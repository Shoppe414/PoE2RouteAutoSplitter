param([Parameter(Mandatory=$true)][string]$ChecklistFile, [string]$OutputLss = "Path of Exile 2 - Area Checklist.lss")
$objectives = @()
Get-Content $ChecklistFile | ForEach-Object {
  $line = ($_ -split '#',2)[0].Trim()
  if ($line -and -not $line.StartsWith('@')) { $objectives += $line }
}
if ($objectives.Count -eq 0) { throw 'No objectives found.' }
[xml]$xml = '<Run version="1.7.0"><GameIcon/><GameName>Path of Exile 2</GameName><CategoryName>Area Checklist</CategoryName><Metadata><Run id=""/><Platform usesEmulator="False">PC</Platform><Region/><Variables/></Metadata><Offset>00:00:00</Offset><AttemptCount>0</AttemptCount><Segments/></Run>'
...
$xml.Save($OutputLss); Write-Host "Created $OutputLss with $($objectives.Count) objective slots."
