...
Entering the `@start` area starts the timer. Thereafter only the next route target can split; out-of-order areas are ignored. The final route target naturally ends the run when the `.lss` row count matches the route.
