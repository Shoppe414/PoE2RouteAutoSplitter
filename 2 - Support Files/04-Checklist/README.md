...
Load the `.lss` that accompanies the preset. On first completion of an unresolved timed objective, the current generic row is renamed to that area and split. Revisits are ignored. When `@start=G1_1` and Riverbank is included in the checklist, Riverbank is counted as satisfied when the Riverbank start gate is armed and therefore does not require its own zero-time LiveSplit row; timing begins on the Wounded Man final opening line. All remaining objectives have one row each.
